...
from catch_the_apple.superpowers import SuperPowerState, object_falling_scale
...
def update_movement(
    ...
    for falling_object in world.falling_objects:
        ...
        object_time_scale = falling_time_scale
        if power_state is not None:
            object_time_scale *= object_falling_scale(
                falling_object.definition.identifier,
                falling_object.definition.category,
                power_state,
            )
        object_delta_time = delta_time * object_time_scale
        if environment_manager is not None:
            target_wind = environment_manager.gameplay_wind_velocity() * wind_scale
            smoothing = environment_manager.wind_response
            falling_object.wind_velocity.x += (
                target_wind.x - falling_object.wind_velocity.x
            ) * min(1.0, smoothing * object_delta_time)
            falling_object.x += falling_object.wind_velocity.x * object_delta_time
